...
type VariantType = 'success' | 'warning' | 'info' | 'error';
type StyleVariantType = 'default' | 'filled';

const AlertToast = React.forwardRef<HTMLDivElement, AlertToastProps>(
  ({ className, variant: variantProp, styleVariant: styleVariantProp, title, description, onClose, ...props }, ref) => {
    const variant: VariantType = variantProp ?? 'info';
    const styleVariant: StyleVariantType = styleVariantProp ?? 'default';
      ...
      >
        ...
        <div className="flex-shrink-0">
          <button
            ...
            aria-label="Close"
            className={cn(
              "p-1 rounded-full opacity-80 hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-offset-2 cursor-pointer",
